...
export function createApp(): Hono<AppEnv> {
  ...
  app.get("/api/health", (c) =>
    c.json({ status: "ok", service: "chalista-api" }),
  );
  ...
  app.notFound((c) =>
    c.json({ error: "not_found", message: "Endpoint tidak ditemukan" }, 404),
  );

  app.onError((error, c) => {
    console.error("[api] unhandled error:", error);
    return c.json(
      { error: "internal_error", message: "Terjadi kesalahan internal" },
      500,
    );
  });
