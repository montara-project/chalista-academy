...
export const zoomRoutes = new Hono<AppEnv>().use('*', requireAuth).post('/signature', async (c) => {
    ...
    return c.json(
      {
        ...
        message:
          'Kredensial Zoom Meeting SDK belum disetel. Isi ZOOM_SDK_KEY dan ZOOM_SDK_SECRET di .dev.vars (dev) atau wrangler secret (produksi).',
      },
      503,
    );
  }
