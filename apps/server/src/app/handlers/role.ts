import { BaseGetParamSchema, PaginationQuerySchema, RolesFormSchema } from '@chalista/types'
...
route.post('/', zValidator('json', RolesFormSchema), async (c) => {
...
route.put(
  ...
  zValidator('json', RolesFormSchema),
  async (c) => {
    const { id } = c.req.valid('param')
    const values = c.req.valid('json')
