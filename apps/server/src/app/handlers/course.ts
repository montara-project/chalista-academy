import { BaseGetParamSchema, CreateCourseRequestSchema, PaginationQuerySchema, UpdateCourseRequestSchema } from '@chalista/types'
...
route.get('/', authorization(), zValidator('query', PaginationQuerySchema), async (c) => {
...
route.post('/', authorization(), zValidator('json', CreateCourseRequestSchema), async (c) => {
...
route.put(
  ...
  zValidator('json', UpdateCourseRequestSchema),
  async (c) => {
    const { id } = c.req.valid('param')
    const values = c.req.valid('json')
...
export { route as CourseHandlers }
